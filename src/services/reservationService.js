...
import {
  reservationFailure, setReservation, reservationCreated, reservationRequest,
} from '../redux/slices/reservation';
import { getReservationsAsync, createReservationAsync } from '../api/reservations';
...
export const getReservation = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch(reservationFailure(error.response ? error.response.data.message : error.message));
